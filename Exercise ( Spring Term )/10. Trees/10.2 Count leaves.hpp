...
int count_leaves_tree(TreeNode* t){
  if(t==NULL){
    ...
    return{
      count_leaves_tree(t->right)+count_leaves_tree(t->left)
    };
  }
}
int main(){
    ...
    std::cout<<"Num of nodes in tree: "<<count_nodes_tree(t)<<std::endl;
    std::cout<<"Num of leaves in tree: "<<count_leaves_tree(t);
    deallocate_tree(t);
